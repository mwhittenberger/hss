...
function post_to_third_party( $entry, $form ) {
    ...
    $first = rgar( $entry, '1' );
    $last = rgar( $entry, '2' );
    $phone = $last = rgar( $entry, '3' );
    $email = $last = rgar( $entry, '4' );
    $city = $last = rgar( $entry, '9' );
    $state = $last = rgar( $entry, '10' );
    $zip = $city = $last = rgar( $entry, '7' );
    ...
    $endpoint_url = 'https://www.nativemediacrm.com/api/post_lead.php?api_key=b1578c1b-57c1-40f5-b334-68ff4c6c83b9&Five9list=Security Website&number1='.$phone.'&first_name='.$first.'&last_name='.$last.'&city='.$city.'&state='.$state.'&zip='.$zip.'&email='.$email.'&IP='.$ip.'&Time_Stamp='.date('n/j/Y g:i:s A').'&LeadID_Token=344555555&Source_URL=http://homesecuritysystems.deals&ANI_Number=344566666&Source_ID=esddttrrre';

    GFCommon::log_debug( 'gform_after_submission: body => ' . print_r( $endpoint_url, true ) );
